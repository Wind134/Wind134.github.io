...
- **关于空间分配**
  ...
  以`main`函数为例，在进入大括号之前就会准备分配空间，以变量为例，在定义变量之前就会为之分配空间。(<font color="blue">后面会有更具体的学习</font>)

- **for循环变量定义范围**：

  `for`循环中定义的变量只作用在大括号内！
...
我们必须注意`pc`所指的真是对象是一个`int`而非字符，如果真的把`pc`当成普通的字符指针使用就可能在运行时发生错误。
